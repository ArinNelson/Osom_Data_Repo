function gen_usgs_file_stats(usgs_id,save_file)
...
% GATHER SITE METADATA & VARIABLES IN INVENTORY
    ...
    while( ~all(cellfun(@isempty,tmp)==1) )
        switch tmp{3}
            ...
            otherwise;      warning(['Unknown or unsupported variable label: ' tmp{3}]);
        end
...
% CREATE THE NETCDF FILE      
    ...
    dimid(1) = netcdf.defDim(ncid,'day',nc_unlimited);
    dimid(2) = netcdf.defDim(ncid,'prctile',7);
    ...
    for i=5:size(var_info,1)
        varid(end+1) = netcdf.defVar(ncid,[var_info{i,1} '_mean'],'NC_DOUBLE',dimid(1));
            netcdf.putAtt(ncid,varid(end),'long_name',[var_info{i,1} ' daily mean']);
            netcdf.putAtt(ncid,varid(end),'units',var_info{i,3});
            netcdf.defVarDeflate(ncid,varid(end),true,true,2);
            netcdf.defVarFill(ncid,varid(end),false,-999);
        varid(end+1) = netcdf.defVar(ncid,[var_info{i,1} '_std'],'NC_DOUBLE',dimid(1));
            netcdf.putAtt(ncid,varid(end),'long_name',[var_info{i,1} ' daily standard deviation']);
            netcdf.putAtt(ncid,varid(end),'units',var_info{i,3});
            netcdf.defVarDeflate(ncid,varid(end),true,true,2);
            netcdf.defVarFill(ncid,varid(end),false,-999);    
        varid(end+1) = netcdf.defVar(ncid,[var_info{i,1} '_num'],'NC_DOUBLE',dimid(1));
            netcdf.putAtt(ncid,varid(end),'long_name',[var_info{i,1} ' daily number of data points']);
            netcdf.putAtt(ncid,varid(end),'units','count');
            netcdf.defVarDeflate(ncid,varid(end),true,true,2);
            netcdf.defVarFill(ncid,varid(end),false,-999); 
        varid(end+1) = netcdf.defVar(ncid,[var_info{i,1} '_cvg'],'NC_DOUBLE',dimid(1));
            netcdf.putAtt(ncid,varid(end),'long_name',[var_info{i,1} ' daily percent coverage of data points']);
            netcdf.putAtt(ncid,varid(end),'units','percent');
            netcdf.defVarDeflate(ncid,varid(end),true,true,2);
            netcdf.defVarFill(ncid,varid(end),false,-999);    
        varid(end+1) = netcdf.defVar(ncid,[var_info{i,1} '_prctile'],'NC_DOUBLE',[dimid(1) dimid(2)]);
            netcdf.putAtt(ncid,varid(end),'long_name',[var_info{i,1} ' daily percentiles']);
            netcdf.putAtt(ncid,varid(end),'percentiles',[var_info{i,1} 'min 10 25 50 75 90 max']);
            netcdf.putAtt(ncid,varid(end),'units',var_info{i,3});
            netcdf.defVarDeflate(ncid,varid(end),true,true,2);
            netcdf.defVarFill(ncid,varid(end),false,-999); 
    end
