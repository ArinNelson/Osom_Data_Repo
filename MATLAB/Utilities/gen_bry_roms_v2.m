...
bry_fileopts.FileName = 'OSOM_bry_2019_DOPPIO_best.nc';
...
if(Switch(03))  
    ...
    disp(['Gathering data for ' num2str(year_on) '/' sprintf('%0.2d',month_on) ', boundary ' str_wsen{i} '...']);
        ...
        for iv=1:n_var
            ...
            switch var_to_get{iv,4}
                case 1
                    ii0  = min(ntrpinfo{1}(:));     nii = max(ntrpinfo{1}(:))-ii0+1;        iii = ii0:(ii0+nii-1);
                    jj0  = min(ntrpinfo{2}(:));     njj = max(ntrpinfo{2}(:))-jj0+1;        jjj = jj0:(jj0+njj-1);
                    in_mask  = in_mask(iii,jjj);
                otherwise
                    ii0_u = min(ntrpinfo_u{1}(:));  nii_u = max(ntrpinfo_u{1}(:))-ii0_u+1;	iii_u = ii0_u : (ii0_u+nii_u-1);
                    jj0_u = min(ntrpinfo_u{2}(:));  njj_u = max(ntrpinfo_u{2}(:))-jj0_u+1;	jjj_u = jj0_u : (jj0_u+njj_u-1);
                    ...
                    ii0_v = min(ntrpinfo_v{1}(:));  nii_v = max(ntrpinfo_v{1}(:))-ii0_v+1;	iii_v = ii0_v : (ii0_v+nii_v-1);
                    jj0_v = min(ntrpinfo_v{2}(:));  njj_v = max(ntrpinfo_v{2}(:))-jj0_v+1;	jjj_v = jj0_v : (jj0_v+njj_v-1);
